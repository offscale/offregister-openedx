    ...
    with cd("$HOME/Downloads"):
        ...
        sudo(
            ...
            ' && cp -r edx-platform "{edx_app_path}"'.format(
                paths=" ".join(['"{}"'.format(p) for p in paths]),
...
def static_collector(*args, **kwargs):
    with cd("/edx/app/edxapp/edx-platform"):
        run(
            "python manage.py lms collectstatic --link --noinput --settings=fun.docker_build_production"
        )
        run(
        ...
        run(
            "python manage.py lms collectstatic --noinput --settings=fun.docker_build_production"
        )
        run(
